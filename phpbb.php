...
if (! function_exists('phpbb_auth_login'))
{
    function phpbb_auth_login($name, $pass, $hashed = false, $expire, $loginadm = false, $return_username = false)
    {
        ...
        if (! $loginadm)
        {
            ...
            define('USER_NAME', $user_info[2]);
            define('USER_MAIL', $user_info[3]);
            define('GROUP_ID', (intval($user_info[5]) == 1 ? 1 : 3));
        }
        ...
        $user_y               = base64_encode(serialize(['id'=>$user_info[1], 'name'=>$user_info[2], 'mail'=>$user_info[3], 'last_visit'=>time()]));
        ...
        if (! $hashed && ! $loginadm)
        {
            $usrcp->kleeja_set_cookie(
                                'ulogu', $usrcp->en_de_crypt($user_info[1] . '|' . $user_info[4] . '|' . $expire . '|' .
                                sha1(md5($config['h_key'] . $user_info[4]) . $expire) . '|' . (intval($user_info[5]) == 1 ? 1 : 3) . '|' . $user_y), $expire
                            );
        }
